...
class StudentBillingFragment : BaseFragment() {
    private lateinit var studentViewModel: StudentViewModel
    private var recyclerView: RecyclerView? = null
    private var billAdapter: FileModelAdapter? = null
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_student_billing, container, false)
    }
        ...
        downloadReceiver = object : BroadcastReceiver() {
            override fun onReceive(context: Context?, intent: Intent?) {
                ...
                downloadList.forEach {
                    if (it.third == id) {
                        studentViewModel.saveDownloadFilePathToDb(it.first, it.second, DBEntities.BILLING)
                    }
                }
            }
        }
    ...
    private fun subscribeObservers() {
        studentViewModel.cachedToken.observe(viewLifecycleOwner, Observer { token ->
            studentViewModel.getStudentBill(token, shouldFetch)
                .observe(viewLifecycleOwner, Observer { resource ->
                    when (resource.status) {
                        Status.SUCCESS -> {
                            showDataAvailableMessage(label_msg_title, resource.data, MessageType.FILES)
                            Timber.i("billing image data :${resource.data?.size}")
                            billAdapter?.submitList(resource?.data)
    ...
    private fun initRecyclerView() {
        ...
        billAdapter = FileModelAdapter()
        billAdapter?.setItemCallback(object : ItemCallback<Triple<ViewFilesAction, Int?, String?>> {
    ...
    private fun downloadWithManager(
        ...
        if (path.exists()) {
            studentViewModel.saveDownloadFilePathToDb(id, path.absolutePath, DBEntities.BILLING)
            return downloadList
        }
    ...
    private fun showDeleteDialog() {
        ...
        alertDialog?.setPositiveButton("yes") { dialog, _ ->
            studentViewModel.deleteFileById(itemData?.second, itemData?.third, DBEntities.BILLING)
            dialog.dismiss()
        }
