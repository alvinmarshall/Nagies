...
class ReportFragment : BaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val extra = arguments?.getBoolean(REPORT_IMAGE_RECEIVE_EXTRA)
        extra?.let { result ->
    ...
    private fun subscribeObservers() {
        studentViewModel.cachedToken.observe(viewLifecycleOwner, Observer { token ->
            shouldFetch.observe(viewLifecycleOwner, Observer { fetch ->
                if (fetch) {
                    preferenceProvider.setNotificationCallback(REPORT_IMAGE_RECEIVE_EXTRA, false)
                }
                studentViewModel.getStudentReportImage(token, fetch).observe(viewLifecycleOwner, Observer { resource ->
    ...
    override fun onResume() {
        super.onResume()
        val perf = preferenceProvider.getNotificationCallback(REPORT_IMAGE_RECEIVE_EXTRA)
        perf?.let {
            if (it) {
                ...
                if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    when (itemData?.first) {
                        ...
                        ViewFilesAction.DOWNLOAD -> {
                            fetchFromServer(itemData?.third)
                        }
